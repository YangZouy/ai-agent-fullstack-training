...
// 修复模式的工具集：在代码理解的 4 个工具之外补上改代码与跑测试。
export function createRepairTools(
  runtime: DemoToolRuntime,
  context: DemoExecutionContext,
): AgentTool[] {
  return [
    ...
    createTool(
      "run_test",
      "Run test",
      "按约定范围运行测试并返回真实退出码。scope=target 目标用例，scope=boundary 到期边界，scope=regression 约定回归。",
      RunTestInput,
      runtime,
      context,
    ),
